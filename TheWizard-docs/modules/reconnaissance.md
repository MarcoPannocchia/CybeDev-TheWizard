...
Reconnaissance module: port scanning (three different implementations), banner grabbing, HTTP fetch, and subdomain enumeration. Unchanged between Version 7 (v0.8.0) and v0.8.2 — re-verified against `ModulesTheWidzard_v082.py`.

---

### `async _port_scanner(self, ip, port, timeout=0.1)`
*Private — not called directly by `TheWizard`.*
...
**Returns:** `None` (side effect: prints `[PORT]`/`[BANNER]` to stdout on success, populates `self.banners`)

**Handled exceptions:**
| Exception | Behavior |
|---|---|
| `asyncio.TimeoutError` | silently ignored (port considered closed/filtered) |
| `ConnectionRefusedError` | silently ignored |
| `OSError` | silently ignored |

> **Doc correction (v0.8.2 re-check)**: earlier documentation passes claimed `ConnectionRefusedError`/`OSError` print `"access denied"`/`"server unreachable"`. Re-reading the actual source (confirmed again in `ModulesTheWidzard_v082.py`) shows all three exceptions are caught with a bare `pass` — a closed, filtered, or unreachable port produces **no output at all**, only open ports (and their banners, if any) are printed. This was a doc/code mismatch, not a behavior change.
